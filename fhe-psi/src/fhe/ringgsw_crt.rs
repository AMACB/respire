...
pub struct Ciphertext<
    ...
    const G_LEN: usize,
> {
    ct: Matrix<N, M, Z_N_CycloRaw_CRT<D, Q1, Q2>>,
}
...
pub struct PublicKey<
    ...
    const G_LEN: usize,
> {
    A: Matrix<N, M, Z_N_CycloRaw_CRT<D, Q1, Q2>>,
}
...
pub struct SecretKey<
    ...
    const G_LEN: usize,
> {
    s_T: Matrix<1, N, Z_N_CycloRaw_CRT<D, Q1, Q2>>,
}
...
impl<
        ...
        const NOISE_WIDTH_MILLIONTHS: u64,
    > FHEScheme<P>
    for RingGSW_CRT<N_MINUS_1, N, M, P, Q, Q1, Q2, D, G_BASE, G_LEN, NOISE_WIDTH_MILLIONTHS>
{
    type Ciphertext = Ciphertext<N, M, P, Q, Q1, Q2, D, G_BASE, G_LEN>;
    type PublicKey = PublicKey<N, M, P, Q, Q1, Q2, D, G_BASE, G_LEN>;
    type SecretKey = SecretKey<N, M, P, Q, Q1, Q2, D, G_BASE, G_LEN>;

    fn keygen() -> (Self::PublicKey, Self::SecretKey) {
        let (A, s_T) = gsw_keygen::<N_MINUS_1, N, M, Z_N_CycloRaw_CRT<D, Q1, Q2>, NOISE_WIDTH_MILLIONTHS>();
        (PublicKey { A }, SecretKey { s_T })
    }

    fn encrypt(pk: &Self::PublicKey, mu: Z_N<P>) -> Self::Ciphertext {
        let mu = Z_N_CycloRaw_CRT::<D, Q1, Q2>::from(u64::from(mu));
        let ct = gsw_encrypt_pk::<N, M, G_BASE, G_LEN, Z_N_CycloRaw_CRT<D, Q1, Q2>>(&pk.A, mu);
        Ciphertext { ct }
    }

    fn encrypt_sk(sk: &Self::SecretKey, mu: Z_N<P>) -> Self::Ciphertext {
        let mu = Z_N_CycloRaw_CRT::<D, Q1, Q2>::from(u64::from(mu));
        let ct = gsw_encrypt_sk::<N_MINUS_1, N, M, G_BASE, G_LEN, Z_N_CycloRaw_CRT<D, Q1, Q2>, NOISE_WIDTH_MILLIONTHS>(&sk.s_T, mu);
        Ciphertext { ct }
    }
    ...
    fn decrypt(sk: &Self::SecretKey, ct: &Self::Ciphertext) -> Z_N<P> {
        let s_T = &sk.s_T;
        let ct = &ct.ct;
        let pt = (&gsw_half_decrypt::<N, M, P, Q, G_BASE, G_LEN, Z_N_CycloRaw_CRT<D, Q1, Q2>>(s_T, ct)).into();
...
impl<
    ...
    const D: usize,
        const G_BASE: u64,
        const G_LEN: usize,
    > CiphertextRef<P, Ciphertext<N, M, P, Q, Q1, Q2, D, G_BASE, G_LEN>>
    for &'a Ciphertext<N, M, P, Q, Q1, Q2, D, G_BASE, G_LEN>
{
}
...
//         const G_BASE: u64,
//         const G_LEN: usize,
//     > Add<&Z_N<P>> for &Ciphertext<N, M, P, Q, Q1, Q2, D, G_BASE, G_LEN>
// {
//     type Output = Ciphertext<N, M, P, Q, Q1, Q2, D, G_BASE, G_LEN>;
//     fn add(self, rhs: &Z_N<P>) -> Self::Output {
//         let rhs_q = &Z_N::<Q>::from(u64::from(*rhs));
//         Ciphertext {
//             ct: &self.ct + &(&build_gadget::<N, M, Q, Q1, Q2, D, G_BASE, G_LEN>() * rhs_q),
//         }
//     }
// }
...
impl<
    ...
    const D: usize,
        const G_BASE: u64,
        const G_LEN: usize,
    > Mul<Z_N<P>> for &'a Ciphertext<N, M, P, Q, Q1, Q2, D, G_BASE, G_LEN>
{
    type Output = Ciphertext<N, M, P, Q, Q1, Q2, D, G_BASE, G_LEN>;
    fn mul(self, rhs: Z_N<P>) -> Self::Output {
        let rhs_q = &Z_N_CycloRaw_CRT::<D, Q1, Q2>::from(u64::from(rhs));
        Ciphertext {
            ct: &self.ct
                * &gadget_inverse::<Z_N_CycloRaw_CRT<D, Q1, Q2>, N, M, M, G_BASE, G_LEN>(
                    &(&build_gadget::<Z_N_CycloRaw_CRT<D, Q1, Q2>, N, M, G_BASE, G_LEN>() * rhs_q),
                ),
        }
    }
}
...
impl<
    ...
    const D: usize,
        const G_BASE: u64,
        const G_LEN: usize,
    > Add for &'a Ciphertext<N, M, P, Q, Q1, Q2, D, G_BASE, G_LEN>
{
    type Output = Ciphertext<N, M, P, Q, Q1, Q2, D, G_BASE, G_LEN>;
...
impl<
    ...
    const D: usize,
        const G_BASE: u64,
        const G_LEN: usize,
    > Mul for &'a Ciphertext<N, M, P, Q, Q1, Q2, D, G_BASE, G_LEN>
{
    type Output = Ciphertext<N, M, P, Q, Q1, Q2, D, G_BASE, G_LEN>;
    fn mul(self, rhs: Self) -> Self::Output {
        Ciphertext {
            ct: &self.ct * &gadget_inverse::<Z_N_CycloRaw_CRT<D, Q1, Q2>, N, M, M, G_BASE, G_LEN>(&rhs.ct),
        }
    }
}
