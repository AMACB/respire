...
pub const RingGSWNTTCRT_TEST_PARAMS: Params = Params {
    N: 2,
    M: 112,
    P: 31,
    Q1: 268369921,
...
pub type RingGSWNTTCRTTest = gsw_from_params!(RingGSWNTTCRT_TEST_PARAMS);
...
mod test {
    ...
    fn keygen_is_correct() {
        let threshold =
            4f64 * (RingGSWNTTCRT_TEST_PARAMS.NOISE_WIDTH_MILLIONTHS as f64 / 1_000_000_f64);
        let (A, s_T) = RingGSWNTTCRTTest::keygen();
        let e = &s_T.s_T * &A.A;

        for i in 0..RingGSWNTTCRT_TEST_PARAMS.M {
            assert!(
